...
def callback(
    ...
    location = Location(
        float(os.environ.get("LATITUDE")), float(os.environ.get("LONGITUDE"))
    )
    pv_power = Photovoltaic(
    ...
    path = (
        ...
        / "results/consumption_report.csv"
    )
    columns = ["dt", "household_consumption", "pv_power", "result_consumption"]
    data = ResultData(
        dt_str, household_consumption, pv_power, result_consumption
    )
